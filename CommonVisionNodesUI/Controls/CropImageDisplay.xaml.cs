...
public sealed partial class CropImageDisplay : UserControl
{
	...
	private void RedrawCropOverlay()
	{
		...
		var (offsetX, offsetY, scaleX, scaleY) = GetImageMapping();
		if (scaleX <= 0)
			return;

		var displayX = offsetX + _cropX / scaleX;
		var displayY = offsetY + _cropY / scaleY;
		var displayW = _cropW / scaleX;
		var displayH = _cropH / scaleY;
	...
	{
		var (offsetX, offsetY, scaleX, scaleY) = GetImageMapping();
		if (scaleX <= 0 || _currentImage is null)
			return (0, 0, 0, 0);
		...
		var imgX = (int)Math.Round((displayX - offsetX) * scaleX);
		var imgY = (int)Math.Round((displayY - offsetY) * scaleY);
		var imgW = (int)Math.Round(displayW * scaleX);
		var imgH = (int)Math.Round(displayH * scaleY);
