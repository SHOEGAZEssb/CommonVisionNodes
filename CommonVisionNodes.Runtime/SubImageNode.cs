...
namespace CommonVisionNodes.Runtime
{
	...
	public sealed class SubImageNode : Node
	{
		...
		public override void EmitHelperMethods(StringBuilder sb)
		{
			...
			sb.AppendLine("{");
			sb.AppendLine("    int sourceWidth = source.Width;");
			sb.AppendLine("    int sourceHeight = source.Height;");
			sb.AppendLine("    int x = Math.Clamp(areaX, 0, Math.Max(0, sourceWidth - 1));");
			sb.AppendLine("    int y = Math.Clamp(areaY, 0, Math.Max(0, sourceHeight - 1));");
			sb.AppendLine("    int w = Math.Clamp(areaWidth, 1, sourceWidth - x);");
			sb.AppendLine("    int h = Math.Clamp(areaHeight, 1, sourceHeight - y);");
			...
			sb.AppendLine("            {");
			sb.AppendLine("                var srcPtr = srcBase + (nint)((y + dy) * srcYInc + (x + dx) * srcXInc);");
