...
public sealed class RuntimeNodeCatalog
{
	...
	public bool TryCreateNode(string type, out Node node)
	{
		node = type switch
		{
			...
			nameof(PlaneMergeNode) => new PlaneMergeNode(),
			nameof(PlaneSplitNode) => new PlaneSplitNode(),
			nameof(PolimagoClassifyNode) => new PolimagoClassifyNode(),
			nameof(MinosSearchNode) => new MinosSearchNode(),
	...
	private IReadOnlyList<NodeDefinitionDto> CreateDefinitions()
	{
		...
		[
			...
			CreateDefinition(
				...
				PreviewToggleProperty()),
			CreateDefinition(
				nameof(PlaneMergeNode),
				"Plane Merge",
				"Processing",
				"Combine individual single-plane images into one multi-plane image.",
				"&#xE8A5;",
				NodePreviewKindDto.None,
				canEditWhileRunning: false,
				() => new PlaneMergeNode(),
				IntProperty(nameof(PlaneMergeNode.PlaneCount), "Plane Count", "Number of image inputs and output planes.", 1, PlaneMergeNode.MaximumPlaneCount, 1),
				StringProperty(nameof(PlaneMergeNode.PlaneWeights), "Plane Weights", "Comma-separated intensity multipliers for the input planes.")),
			CreateDefinition(
				nameof(PlaneSplitNode),
				"Plane Split",
				"Processing",
				"Copy each input plane into an individual single-plane image output.",
				"&#xE8A5;",
				NodePreviewKindDto.None,
				canEditWhileRunning: false,
				() => new PlaneSplitNode(),
				IntProperty(nameof(PlaneSplitNode.PlaneCount), "Plane Count", "Number of input planes and output ports.", 1, PlaneSplitNode.MaximumPlaneCount, 1),
				EnumProperty<PlaneSplitMode>(nameof(PlaneSplitNode.Mode), "Pixel Mode", "Copy plane pixels into independent images or link output planes to the input image.")),
			CreateDefinition(
				nameof(CSharpNode),
				"C# Script",
				"Processing",
			...
			CreateDefinition(
				...
				PreviewToggleProperty(defaultValue: true)),
			CreateDefinition(
				nameof(MinosSearchNode),
				"Minos Search",
				"Analysis",
				"Locate learned patterns with a Minos classifier.",
				"&#xE721;",
				NodePreviewKindDto.Classification,
				canEditWhileRunning: true,
				() => new MinosSearchNode(),
				StringProperty("ClassifierPath", "Classifier File", "Path to the Minos .clf file."),
				EnumProperty<MinosSearchOperation>("SearchOperation", "Search", "Whether to find all, first, or best matches."),
				DoubleProperty("Density", "Density", "Fraction of candidate pixels to scan.", 0, 1, 0.05),
				DoubleProperty("MinQuality", "Min Quality", "Minimum normalized match quality.", 0, 1, 0.05),
				IntProperty("Locality", "Locality", "Find All suppression radius in pixels.", 0, null, 1),
				IntProperty("MaxResults", "Max Results", "Maximum number of exposed results. 0 disables the limit.", 0, null, 1),
				PreviewToggleProperty()),
			CreateDefinition(
